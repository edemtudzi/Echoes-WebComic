    ...
    <main className="view admin-dashboard">
      <section className="section-head admin-hero">
        <div>
          ...
          <h2>Analytics and production control.</h2>
          <p>Track reader growth, engagement, unlock flow, reflections, and publishing health from one screen.</p>
        </div>
        <div className="actions" style={{ marginTop: 0 }}>
          <Link className="button" href="/admin/comics">Manage Comics</Link>
      ...
      <section className="analytics-grid">
        <article className="analytics-panel">
          <div className="eyebrow">Reader Funnel</div>
          ...
          <div className="funnel-list">
            {funnelSteps.map((step, index) => (
              <div className="funnel-step" key={step.label}>
                ...
                <small>{index ? `${pct(step.value, funnelSteps[index - 1].value)} from previous` : "Starting audience"}</small>
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="analytics-panel">
          ...
          <div className="bar-list">
            {Object.entries(reactionLabels).map(([key, label]) => {
              ...
              return <div className="bar-row" key={key}><div><span>{label}</span><strong>{fmt(total)}</strong></div><i style={{ width: pct(total, metrics.total_reflections) }} /></div>;
            })}
          </div>
        </article>
      ...
      <style>{`
        .admin-dashboard{display:grid;gap:24px}.admin-hero{margin-bottom:0}.metric-grid,.analytics-grid{display:grid;gap:18px}.metric-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.analytics-grid{grid-template-columns:repeat(2,minmax(0,1fr));align-items:start}.metric-card,.analytics-panel{border:1.5px solid rgba(9,9,9,.16);background:rgba(255,254,248,.88);box-shadow:var(--shadow-card)}.metric-card{min-height:150px;padding:20px;border-radius:var(--radius-lg);display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden}.metric-card:after{content:"";position:absolute;right:-32px;bottom:-42px;width:126px;height:126px;border-radius:50%;background:radial-gradient(circle,rgba(255,210,26,.58),transparent 68%)}.metric-card span,.status-grid span,.funnel-step span,.activity-item span{color:var(--muted);font-size:12px;font-weight:850;text-transform:uppercase;letter-spacing:.08em}.metric-card strong{display:block;margin-top:12px;font-size:clamp(34px,5vw,58px);line-height:.9;letter-spacing:-.055em}.metric-card p,.activity-item p{margin:10px 0 0;color:var(--muted)}.analytics-panel{border-radius:var(--radius-lg);padding:clamp(18px,3vw,26px);overflow:hidden}.analytics-panel h3{margin-bottom:16px}.funnel-list,.bar-list,.activity-list,.status-grid{display:grid;gap:12px}.funnel-step,.activity-item,.status-grid div{border:1px solid rgba(9,9,9,.12);border-radius:22px;background:rgba(247,245,235,.72);padding:15px}.status-grid div{background:linear-gradient(90deg,rgba(255,210,26,.34),transparent 44%),rgba(247,245,235,.72)}.funnel-step div,.bar-row div{display:flex;align-items:baseline;justify-content:space-between;gap:12px}.funnel-step strong,.status-grid strong{font-size:30px;line-height:1;letter-spacing:-.04em}.funnel-step small,.status-grid small,.activity-item small{display:block;margin-top:8px;color:var(--dim);line-height:1.4}.status-grid strong{display:block;margin-top:8px}.bar-row{display:grid;gap:8px}.bar-row span{color:var(--muted);font-weight:850}.bar-row i{display:block;max-width:100%;height:10px;border-radius:999px;background:linear-gradient(90deg,var(--yellow),var(--yellow-deep));box-shadow:inset 0 0 0 1px rgba(9,9,9,.14)}.table-wrap{width:100%;overflow-x:auto;border:1px solid rgba(9,9,9,.12);border-radius:24px;background:rgba(255,254,248,.74)}.analytics-table{width:100%;min-width:900px;border-collapse:collapse}.analytics-table th,.analytics-table td{padding:14px 15px;text-align:left;border-bottom:1px solid rgba(9,9,9,.10);vertical-align:top}.analytics-table th{color:var(--ink);background:rgba(255,210,26,.46);font-size:12px;text-transform:uppercase;letter-spacing:.08em}.analytics-table tr:last-child td{border-bottom:0}.analytics-table td{color:var(--muted)}.analytics-table td strong{display:block;color:var(--ink)}.analytics-table td span{display:block;margin-top:4px;font-size:12px;color:var(--dim)}.activity-item{display:grid;gap:4px}.activity-item strong{font-size:16px}@media(max-width:980px){.metric-grid,.analytics-grid{grid-template-columns:1fr}}@media(max-width:640px){.metric-card{min-height:132px}.analytics-panel{padding:16px;border-radius:28px}.funnel-step div,.bar-row div{align-items:start;flex-direction:column;gap:4px}}
      `}</style>
    </main>
  );
}
